...
import '../../widgets/home/ai_activity_suggestions.dart';
...
class _HomeScreenRefactoredState extends State<HomeScreenRefactored>
    ...
    return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              // 🎯 AI Activity Suggestions (Values Mode Only)
              if (!isViceMode) _buildAIActivitySuggestions(isDarkMode),
  ...
  /// 🎯 AI ACTIVITY SUGGESTIONS: Smart recommendations based on values
  Widget _buildAIActivitySuggestions(bool isDarkMode) {
    return BlocBuilder<ValuesBloc, ValuesState>(
      builder: (context, valuesState) {
        ...
        return AIActivitySuggestions(
          values: values,
          onActivityTap: (activityName) {
            // Navigate to activity creation with pre-filled name
            context.go('/activities/new?name=${Uri.encodeComponent(activityName)}');
          },
        );
      },
    );
